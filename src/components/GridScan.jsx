...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.current, 0, { v: tiltVel.current }, smoothTime, maxSpeed, dt);
      tiltCurrent.current = tiltSm.value;
      tiltVel.current = tiltSm.v;

      const yawSm = smoothDampFloat(yawCurrent.current, 0, { v: yawVel.current }, smoothTime, maxSpeed, dt);
      ...
      material.uniforms.uYaw.value = Math.min(0.6, Math.max(-0.6, yawCurrent.current * yawScale));
    ...
    lineStyle, lineJitter, scanDirection, enablePost,
  ]);
