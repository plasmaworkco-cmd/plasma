...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <nav className={`hidden md:flex fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-out ${scrolled ? 'pt-4' : 'pt-8'}`}>
        <div 
          className={`mx-auto max-w-5xl flex items-center justify-between transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]
            ${scrolled 
              ? 'bg-secondary-black/80 backdrop-blur-xl rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-mint/10 py-3 px-6 w-[90%] lg:w-[60rem]' 
        ...
        >
          ...
          <div className="flex items-center gap-1 group cursor-pointer">
            <span className="text-2xl font-black tracking-tighter text-primary-text font-heading">
          ...
          <div className="flex items-center gap-8">
            {['Solutions', 'The Stack', 'Work'].map((item) => (
              <a key={item} href={`#${item.replace(' ', '-').toLowerCase()}`} 
                 className="text-sm font-medium text-muted-text hover:text-emerald transition-all relative group font-body tracking-wide">
                {item}
      ...
      {/* Mobile Top Branding */}
      <div className="md:hidden fixed top-0 left-0 w-full z-40 px-6 py-5 bg-deep-black/80 backdrop-blur-lg flex justify-center items-center border-b border-white/5">
         <span className="text-xl font-black tracking-tighter text-primary-text font-heading">
            PLASMA<span className="text-emerald">.</span>
         </span>
      </div>

      {/* Mobile Bottom Navigation Bar (Thumb-friendly) */}
      <nav className="md:hidden fixed bottom-6 left-4 right-4 z-50 bg-secondary-black/90 backdrop-blur-2xl border border-mint/10 rounded-2xl p-3 shadow-[0_10px_40px_rgba(0,0,0,0.5)] flex justify-between items-center">
        
        {/* Mobile Menu Links */}
        <div className="flex gap-6 pl-4">
           {['Work', 'Stack'].map((item) => (
             <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold text-muted-text hover:text-primary-text transition-colors font-body">
               {item}
             </a>
           ))}
        </div>
