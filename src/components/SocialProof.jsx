...
const SocialProof = () => {
  return (
    <section className="bg-primary py-10 border-b border-mint/5">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center md:justify-start gap-8 md:gap-12 items-center opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
          {['FORBES', 'TECHCRUNCH', 'STRIPE', 'VERCEL', 'AWS'].map((brand) => (
            <span key={brand} className="text-text-primary font-bold text-2xl font-heading hover:text-emerald transition-colors duration-300 cursor-default">
              {brand}
            </span>
          ))}
        </div>
